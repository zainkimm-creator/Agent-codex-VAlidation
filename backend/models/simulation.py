...
from .r2r_dynamics import R2RDynamicsParams, r2r_derivatives, surface_velocities
...
def _to_dynamics_params(params: R2RParameters) -> R2RDynamicsParams:
    """Map legacy simulation/controller params to the focused dynamics params."""

    return R2RDynamicsParams(
        EA_N=params.EA,
        L_m=params.span_length_m,
        R_m=params.roller_radius_m,
        J_kgm2=params.inertia_kg_m2,
        f_Nms_per_rad=params.kf,
        v_ref_mps=params.feeder_velocity_m_s,
    )
...
def simulate(
    ...
    held_motor_torques = (0.0, 0.0, 0.0)
    rows: list[dict[str, float]] = []
    next_control_s = 0.0
    next_log_s = 0.0
    ...
    for step in range(step_count + 1):
        ...
        if t_s + 1e-12 >= next_control_s:
            ...
            held_motor_torques = _add_vectors(action.motor_torque_Nm, excitation_fn(t_s))
        ...
        if t_s + 1e-12 >= next_log_s:
            ...
            dynamics_params = _to_dynamics_params(current_params)
            v_uw, v_nip, v_rw = surface_velocities(measured_state, dynamics_params)
            row = {"time_s": t_s}
            row.update(as_named_dict(measured_state, STATE_NAMES))
            row.update(as_named_dict(held_motor_torques, INPUT_NAMES))
    ...
    equation_sample = as_named_dict(
        r2r_derivatives(equation_state, equation_inputs, _to_dynamics_params(active_params)),
        STATE_NAMES,
    )
