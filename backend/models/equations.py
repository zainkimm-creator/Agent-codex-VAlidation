...
Units:
    ...
    Roller command input u_i: motor torque in N*m
...
def equation_summary() -> dict[str, object]:
    ...
    summary = {
        ...
        "theory_summary": [
            {
                "title": "Plant model",
                "detail": "The paper models a three-span R2R line with three span tensions and three roller speeds. Tension is the measured output, while motor torque or motor-torque commands drive the rollers.",
            },
            {
        ...
        "paper_equations": [
            {
                ...
                "dashboard_note": "The backend uses the same state and output order. Its input vector is a held motor-motor-torque command that is converted through backend motor gains.",
            },
            {
                "number": "(1)",
            ...
            },
            {
                ...
                "equation": "v_corr,i = (L_i/EA)Kp_star [sigma_i e_i + (1/TI) integral_0^t sigma_i e_i(tau) d tau]; omega_ref,i = omega_ss,i + rho_i*v_corr,i/R_i",
                "variables": "e_i = T_ref,i - T_meas,i; sigma = [-1,+1,+1]; rho = [-1,+1,+1] maps actuator correction to positive web-surface direction; Kp_star = Kp*EA/L.",
                ...
                "dashboard_note": "The backend keeps the professor sigma convention and isolates the UW actuator-to-web sign in rho before the inner velocity loop computes motor torque.",
            },
            {
                "number": "(4)",
                ...
                "dashboard_note": "The backend uses velocity feedback plus feedforward torque and outputs motor_torque_Nm directly.",
            },
            {
                "number": "(5)",
            ...
            },
            {
                ...
                "dashboard_note": "The backend estimates the same seven displayed parameter names as paper ratios and logs motor torque directly.",
            },
            {
                "number": "(7)",
        ...
        "backend_equations": [
            ...
            },
            {
                ...
                "equation": "v_corr,i = (L_i/EA)Kp_star(sigma_i*e_i + I_i/TI); I_i[k+1] = I_i[k] + sigma_i*e_i*Ts",
                "variables": "e_i: tension error, sigma = [-1,+1,+1], TI: integral time.",
                "backend_use": "Used by CascadePIController.update() to build signed tension corrections.",
            },
            {
                "title": "Velocity reference sign mapping",
                "equation": "omega_ref,i = omega_ss,i + rho_i*v_corr,i/R_i,  rho = [-1,+1,+1]",
                "variables": "rho maps the UW actuator correction into the positive web-surface convention used by v_i = R_i*omega_i.",
                "backend_use": "Used by the controller to preserve the professor sigma convention while driving the plant's web-positive state.",
            },
            {
                "title": "Inner velocity P plus feedforward",
                "equation": "u_i = Kvel_i(omega_ref,i - omega_i) + u_ff,i; u_ff,i = R_i(T_i - T_{i+1}) + f_i*omega_i",
                "variables": "u_i is motor_torque_Nm; Kvel_i = alpha*J_i*sqrt(EA*R_i^2/(J_i*L_i)).",
                "backend_use": "Used by CascadePIController.update() to produce the held motor torque.",
            },
            {
            ...
            },
            {
                ...
                "equation": "control_effort_rms_Nm = sqrt(sum(u_UW^2 + u_Nip^2 + u_RW^2) / (3N))",
