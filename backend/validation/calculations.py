...
def simulation_calculation_payload(
    ...
    calculations = [
        ...
        _calculation(
            ...
            parameter="control_effort_rms_Nm",
            ...
            result=f"control_effort_rms_Nm = {_format_number(effort_rms)} N*m",
            summary="This shows how much motor-torque effort the controller used while regulating the web.",
            steps=[
                f"Read u_UW, u_Nip, and u_RW from every one of the {len(rows)} logged rows.",
                "Square each motor-torque command so positive and negative effort contribute equally.",
                f"Add all squared motor-torque commands: sum(u^2) = {_format_number(effort_sse)} (N*m)^2.",
                f"Divide by the number of command samples: {effort_terms}.",
                f"Take the square root to get control_effort_rms_Nm = {_format_number(effort_rms)} N*m.",
            ],
        ),
        _calculation(
...
def drift_calculation_payload(payload: Mapping[str, object]) -> dict[str, object]:
    ...
    if dominant:
        ...
        effort = float(dominant["control_effort_rms_Nm"])
