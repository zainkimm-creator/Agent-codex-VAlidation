...
def _excitation_comparison(targets: Mapping[str, Any]) -> dict[str, Any]:
    ...
    dashboard = {
        row.get("excitation_type"): _float(row.get("RMSE_theta_percent"))
        for row in rows
        if row.get("skipped") == "False"
    }
    paper = targets.get("excitation_targets", {}).get("SN_RMSE_theta_percent", {})
    comparison_rows: list[dict[str, Any]] = []
    for name, paper_value_raw in paper.items():
        if name == "E_Toggle":
            continue
        paper_value = _float(paper_value_raw)
        dashboard_value = dashboard.get(name)
        delta = dashboard_value - paper_value if dashboard_value is not None and paper_value is not None else None
        comparison_rows.append(
            {
                "excitation": name,
                "paper_SN_RMSE_theta_percent": paper_value,
    ...
    plot_path = _write_paper_grouped_bar_chart(
        ...
        paper_key="paper_SN_RMSE_theta_percent",
        dashboard_key="dashboard_RMSE_theta_percent",
        y_label="RMSE_theta (%)",
    )
    skipped = targets.get("excitation_targets", {}).get("skipped_for_reproduction", [])
    points = [
        f"Exact mode skips: {', '.join(str(item) for item in skipped)}.",
        "Paper SN ranking favors multi-channel/toggle excitation over ET1.",
    ]
